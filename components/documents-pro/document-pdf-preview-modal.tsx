...
import {
  generateProfessionalDocumentPDF,
  generateProfessionalDocumentPDFBlob,
} from '@/lib/pdf/professional-document-pdf'
...
export function DocumentPDFPreviewModal({
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      const blob = await generateProfessionalDocumentPDFBlob(documentData)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      await generateProfessionalDocumentPDF(documentData)
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-xl font-light">
                Aperçu PDF - {documentData.documentTitle}
              </DialogTitle>
            </div>
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-gray-500">
                  Préparation du document professionnel avec filigrane
                </p>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white">
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div className="flex items-center gap-6">
              <span>N° {documentData.documentNumber}</span>
              {documentData.projectName && (
                <span>Projet : {documentData.projectName}</span>
              )}
              {documentData.client && (
                <span>Client : {documentData.client.name}</span>
              )}
            </div>
